...
namespace {
  ...
  void Adapter::run() {
    ...
    bool isAsyncRunTaskActive = false;
    ...
    while (!stopped) {
      ...
      if (countIterations==10) {
        ...
        isAsyncRunTaskActive = true;
      }

      // Here the main thread can do other work while the asyncRunTask is busy
      if (isAsyncRunTaskActive) {
        std::chrono::milliseconds timeout(100);
        std::cout << "Main task doing something while waiting for asyncRunTask.\n";
        while (asyncStatus.wait_for(timeout) == std::future_status::timeout) {
          // This simulates other work while waiting
          std::cout << "."; std::cout.flush();
          std::this_thread::sleep_for(std::chrono::milliseconds(100));
        }

        std::cout << "\n";
        isAsyncRunTaskActive = false;
      }
    }
...
int main(int argc, char **argv) {
  std::cout << "==========================================================================" << std::endl;
  std::cout << "=== ASYNC/SYNC EXAMPLE 02                                             ====" << std::endl;
